...
```{r auc-all-drug-model, fig.width = 10, fig.height = 9}
...
```{r ic50-all-drug-scaled-model, fig.width = 10, fig.height = 9}
modelSummarizedPharmacoData %>%
   ggplot(aes(x = -log10(ic50_GDSC / 10^6),
              y = -log10(ic50_CCLE / 10^6))) +
    geom_point(alpha= 1/2, cex = 1/2) + 
...
The Pearson correlation coefficient measures the degree of linear between variables, while Spearman's correlation coefficient measures the agreement of the rankings between variables. For interval scale, the Pearson correlation coefficient is typically more appropriate. Therefore, we believe that the Pearson coefficient is more appropriate measure of correlation for this data.  

7. We have explored Pearson and Spearman correlation, but what about other correlation measures? For example, you could try out distance correlation, which is sensitive to nonlinear relationships. Then, you can compute distance correlation with the `dcor()` function. How does this correlation measure compare to Pearson and Spearman? Do your conclusions about the agreement between the two studies change at all using this measure?

```{r distance-corr-summarizeddata}
AUCdistancecorrsum <- dcor(summarizedPharmacoData$auc_GDSC,summarizedPharmacoData$auc_CCLE, index = 1)
```
