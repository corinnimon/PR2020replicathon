...
AUCdrugCorrsmodel <- modelSummarizedPharmacoData %>% 
    group_by(drug) %>%
    summarize(Pearson_auc_model  = cor(auc_GDSC, auc_CCLE, method = "pearson"),
              Spearman_auc_model = cor(auc_GDSC, auc_CCLE, method = "spearman"))
...
IC50drugCorrsmodel <- modelSummarizedPharmacoData %>% 
    group_by(drug) %>%
    summarize(Pearson_ic50_model  = cor(-log10(ic50_GDSC / 10^6), -log10(ic50_CCLE / 10^6), method = "pearson"),
              Spearman_ic50_model = cor(-log10(ic50_GDSC / 10^6), -log10(ic50_CCLE / 10^6), method = "spearman"))
